...
fn check_valid(game: &Game, max_red: u64, max_blue: u64, max_green: u64) -> bool {
    game.rounds.iter().all(| round | round.red <= max_red && round.green <= max_green && round.blue <= max_blue)
}
...
fn main() {
    ...
    let res = games.iter().filter(|&game| check_valid(game,12, 14,13)).map(|x| x.id).sum::<u64>();
